...
function calculateConfidence(evidence: AggregatedEvidence, verdict: VerdictLabel): number {
  // Insufficient evidence still gets a calibrated confidence range
  // so it can reflect source volume/quality instead of always reading 10%.
  if (verdict === 'INSUFFICIENT_EVIDENCE') {
    return calculateInsufficientConfidence(evidence);
  }
...
function calculateInsufficientConfidence(evidence: AggregatedEvidence): number {
  const { supporting, contradicting, inconclusive, totalSources, consensusScore } = evidence;
  if (totalSources === 0) {
    return THRESHOLDS.MIN_CONFIDENCE;
  }

  const allEvidence = [...supporting, ...contradicting, ...inconclusive];
  const evaluableCount = supporting.length + contradicting.length;
  const signalRatio = evaluableCount / totalSources;
  const consensusStrength = Math.abs(consensusScore);
  const highAuthorityRatio =
    allEvidence.filter((item) => item.authority >= 0.8).length / totalSources;

  let confidence = THRESHOLDS.MIN_CONFIDENCE;
  confidence += Math.min(0.18, totalSources * 0.012);
  confidence += signalRatio * 0.12;
  confidence += consensusStrength * 0.1;
  confidence += highAuthorityRatio * 0.1;

  if (evaluableCount === 0) {
    confidence *= 0.75;
  }

  confidence = Math.max(THRESHOLDS.MIN_CONFIDENCE, Math.min(0.45, confidence));
  return Math.round(confidence * 100) / 100;
}
...
function selectCitations(evidence: AggregatedEvidence, verdict: VerdictLabel): Citation[] {
  ...
  // Sort by authority and take a richer set for timeline/map exploration in UI.
  const sorted = relevantEvidence.sort((a, b) => b.authority - a.authority);
  const maxCitations = verdict === 'INSUFFICIENT_EVIDENCE' ? 12 : 8;
  const topSources = sorted.slice(0, maxCitations);
  ...
  return topSources.map((e) => ({
    ...
    stance: e.stance,
    entailmentProvider: e.entailmentProvider,
...
function generateExplanation(
  ...
  switch (verdict) {
    case 'SUPPORTED':
      return (
        `This claim appears to be ${confidenceLevel} supported. ` +
    ...
    case 'FALSE':
      return (
        `This claim appears to be ${confidenceLevel} false. ` +
    ...
    case 'MISLEADING':
      return (
        `This claim is misleading. While some elements may be accurate, ` +
    ...
    default:
      ...
      if (supporting.length === 0 && contradicting.length === 0) {
        return (
          `Unable to determine this claim with confidence. ${totalSources} ` +
          `source${totalSources > 1 ? 's were' : ' was'} analyzed, but none provided a clear support/contradict signal.`
        );
      }
      return (
        `Unable to determine the accuracy of this claim with confidence. ` +
...
function checkSourceDiversity(evidence: AggregatedEvidence): string[] {
  ...
  const fallbackCount = allEvidence.filter(
    (item) =>
      item.entailmentProvider === 'heuristic_fallback' ||
      item.entailmentProvider === 'regex_minimal'
  ).length;
  if (fallbackCount >= Math.ceil(allEvidence.length * 0.6)) {
    warnings.push(
      'Most sources were classified by fallback heuristics because entailment was unavailable.'
    );
  }
