...
export interface EvidenceClassificationOverride {
 ...
 *
 * This intentionally keeps deterministic logic minimal.
 * Main stance classification should come from the entailment providers
 * (on-device NLI / LLM), not regex semantics.
 *
...
export function detectStance(claim: Claim, content: string): EvidenceStance {
  ...
  const contentSentences = splitIntoAnalysisSentences(normalizedContent);
  const relevantSentences = contentSentences.filter(
    (sentence) =>
      calculateRelevance(claimKeywords, sentence) >= 0.2 || detectExplicitVerdict(sentence) !== null
  );
  const analysisContent =
    relevantSentences.length > 0 ? relevantSentences.join(' ') : normalizedContent;

  // First check if content is even relevant to the claim
  const relevanceScore = calculateRelevance(claimKeywords, analysisContent);
  if (relevanceScore < 0.2) {
    return 'INCONCLUSIVE';
  }

  // Explicit fact-check verdict markers are deterministic and high precision.
  const explicitVerdict = detectExplicitVerdict(analysisContent);
  if (explicitVerdict) {
    return explicitVerdict;
  }

  // Numeric contradiction is retained as a deterministic safety check.
  const numericContradiction = detectNumericContradiction(normalizedClaim, analysisContent);
  if (numericContradiction === 'CONTRADICTS') {
    return 'CONTRADICTS';
  }

  // Defer to NLI/LLM for semantic stance; deterministic layer stays conservative.
  return 'INCONCLUSIVE';
}
function splitIntoAnalysisSentences(text: string): string[] {
  return text
    .split(/(?<=[.!?])\s+|\n+/)
  ...
  for (const claimNum of claimNumbers) {
    ...
    for (const contentNum of contentNumbers) {
      ...
      if (
        ...
        if (
          claimNum.value !== contentNum.value &&
...
function detectExplicitVerdict(content: string): EvidenceStance | null {
  ...
  if (
    /\b(false|pants on fire|mostly false)\b/i.test(content) &&
    /\b(rating|verdict|ruling)\b/i.test(content)
  ) {
    return 'CONTRADICTS';
  }
...
export function classifyEvidence(
  ...
  return {
    ...
    entailmentProvider: override?.provider,
    entailmentConfidence: override?.confidence,
  };
}
...
export function processSearchResults(
  ...
  overrides?: Map<string, EvidenceClassificationOverride>
): AggregatedEvidence {
  // Classify each search result
  const evidence = searchResults.map((result) =>
    classifyEvidence(claim, result, overrides?.get(result.url))
  );
